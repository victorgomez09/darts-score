...
function LocalRoundTheClockGame({
  throwsRemaining,
  currentPlayerIndex,
  ...props
}: LocalRoundTheClockGameProps) {
  const [players] = useState<string[]>(props.players);
  const [playerStats, setPlayerStats] = useState<PlayerToPlayerStatsRCl>(() =>
  ...
  const increaseTargetByOne = (playerIndex: number): void => {
    ...
    setPlayerStats((prevPlayerStats) => ({
      ...
      [playerKey]: {
        ...
        lastThrows: beginningOfRound
          ? ["Hit"]
          : [...prevPlayerStats[playerKey].lastThrows, "Hit"],
      },
    }));
  };
  ...
  const addToLastThrows = (playerIndex: number, lastThrows: string[]): void => {
    ...
    setPlayerStats((prevPlayerStats) => ({
      ...
      [playerKey]: {
        ...
        lastThrows: beginningOfRound
          ? [...lastThrows]
          : [...prevPlayerStats[playerKey].lastThrows, ...lastThrows],
      },
    }));
  };
